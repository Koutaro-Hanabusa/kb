...
pub fn parse(input: &str) -> Result<Zoned> {
    ...
    if let Some(span) = relative(input)? {
        return Zoned::now().checked_sub(span).context("--since is too far in the past");
    }
...
fn relative(input: &str) -> Result<Option<Span>> {
    ...
    let amount: i64 = digits.parse().with_context(|| format!("bad duration `{input}`"))?;
...
mod tests {
    ...
    fn reads_absolute_dates() {
        ...
        assert_eq!(parse("2026-04-20T13:34:09+09:00").unwrap().date().to_string(), "2026-04-20");
    }
